...
class Field:
    ...
    def cpu_seg(self, coef_grid=0.2):
        self.set_anchors()
        self.cpu_pre_dim()
    ...
    def set_anchors(self, center=0):
        ...
        for row in range(self.nrow):  # then y
            ...
            for col in range(self.ncol):  # start from x
                ...
                try:
                    sc_W = self.img_binsm[
                        (pos_y - rg_y) : (pos_y + rg_y), (pos_x - rg_x) : pos_x
                    ].sum()
                except:
                    sc_W = 0
                try:
                    sc_E = self.img_binsm[
                        (pos_y - rg_y) : (pos_y + rg_y), pos_x : (pos_x + rg_x)
                    ].sum()
                except:
                    sc_E = 0
                try:
                    sc_N = self.img_binsm[
                        (pos_y - rg_y) : pos_y, (pos_x - rg_x) : (pos_x + rg_x)
                    ].sum()
                except:
                    sc_N = 0
                try:
                    sc_S = self.img_binsm[
                        pos_y : (pos_y + rg_y), (pos_x - rg_x) : (pos_x + rg_x)
                    ].sum()
                except:
                    sc_S = 0
    ...
    def cpu_bound(self, coef_grid=0.2):
        """ """
        for row in range(self.nrow):
            for col in range(self.ncol):
            ...
            for col in range(self.ncol):
                ...
                for dir in list([Dir.EAST, Dir.SOUTH]):
                    ...
                    if agent_neig:
                        ...
                        agent_neig.border[dir_neig.name] = agent_neig.border_reset[
                            dir_neig.name
                        ]
                        # calculate border
                        dist_agents = (
                            abs(agent_self.x - agent_neig.x)
                            if dir == Dir.EAST
                            else abs(agent_self.y - agent_neig.y)
                        )
                        while (
                            abs(
                                agent_self.get_border(dir)
                                - agent_neig.get_border(dir_neig)
                            )
                            > 1
                        ):
    ...
    def denoise(self, img, n_denoise=1):
        """ """
        k_blur = np.array(([1, 4, 1], [4, 9, 4], [1, 4, 1]), dtype="int") / 29
    ...
    def get_index(
        ...
        if isContrast:
            img_index = (img_raw[:, :, ch_1] - img_raw[:, :, ch_2]) / (
                img_raw[:, :, ch_1] + img_raw[:, :, ch_2] + 1e-8
            )
        if isThree:
            img_index = (
                2 * img_raw[:, :, ch_1] - img_raw[:, :, ch_2] - img_raw[:, :, ch_3]
            ) / (img_raw[:, :, ch_1] + img_raw[:, :, ch_2] + img_raw[:, :, ch_3] + 1e-8)
        ...
        for i in self.ls_bin:
            ...
            for row in range(self.nrow):
                for col in range(self.ncol):
                    ...
                    entry = dict(var=agent.name, index=0)
                    rg_row = range(
                        agent.get_border(Dir.NORTH), agent.get_border(Dir.SOUTH)
                    )
                    rg_col = range(
                        agent.get_border(Dir.WEST), agent.get_border(Dir.EAST)
                    )
                    img_bin_agent = self.img_bin[rg_row, :][:, rg_col]
                    img_index_agent = img_index[rg_row, :][:, rg_col]
                    n_veg = img_bin_agent.sum()
