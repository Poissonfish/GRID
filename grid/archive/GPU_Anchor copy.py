...
class Panel_Anchor(QWidget):
    ...
    def initUI(self):
        ...
        """image"""
        qImg = QImage(
            self.img, self.imgW, self.imgH, self.imgW * 1, QImage.Format_Indexed8
        )
    ...
    def mouseReleaseEvent(self, event):
        ...
        # add new anchors
        if (
            (pos.x() == self.pos_temp.x())
            & (pos.y() == self.pos_temp.y())
    ...
    def mouseMoveEvent(self, event):
        ...
        if event.buttons() == Qt.LeftButton:
            if self.rec_acr_c.contains(pos):
                self.acr_c[self.idx_click] = (
                    pos.x() - correctX
                ) / self.size_img.width()
            elif self.rec_acr_r.contains(pos):
                self.acr_r[self.idx_click] = (
                    pos.y() - correctY
                ) / self.size_img.height()
            self.repaint()
    ...
    def paintEvent(self, paint_event):
        ...
        self.size_imgPan = QSize(
            self.size_self.width() - self.margin, self.size_self.height() - self.margin
        )
        ...
        if self.size_img.width() == self.size_imgPan.width():
            ...
            self.pt_st_img = int(
                (self.size_imgPan.height() - self.size_img.height()) / 2
            )
            painter.drawPixmap(
                0,
                self.pt_st_img,
            ...
            """anchor X"""
            self.x_acr_c = (self.acr_c * self.size_img.width()).astype(int)
            self.y_acr_c = self.pt_st_img + self.size_img.height() + (self.margin / 5)
            """anchor Y"""
            self.x_acr_r = self.size_self.width() - (self.margin * 4 / 5)
            self.y_acr_r = (
                self.acr_r * self.size_img.height() + self.pt_st_img
            ).astype(int)
            """rect"""
            self.rec_acr_c = QRect(
                0,
                self.pt_st_img + self.size_img.height() + self.space,
                self.size_img.width(),
                self.margin,
            )
            self.rec_acr_r = QRect(
                self.size_img.width() + self.space,
                self.pt_st_img,
                self.margin,
                self.size_img.height(),
            )
            ...
            """anchor X"""
            self.x_acr_c = (self.acr_c * self.size_img.width() + self.pt_st_img).astype(
                int
            )
            self.y_acr_c = self.size_self.height() - (self.margin * 4 / 5)
            """anchor Y"""
            self.x_acr_r = self.pt_st_img + self.size_img.width() + (self.margin / 5)
            self.y_acr_r = (self.acr_r * self.size_img.height()).astype(int)
            """rect"""
            self.rec_acr_c = QRect(
                self.pt_st_img,
                self.size_img.height() + self.space,
                self.size_img.width(),
                self.margin - 5,
            )
            self.rec_acr_r = QRect(
                self.pt_st_img + self.size_img.width() + self.space,
                0,
                self.margin - 5,
                self.size_img.height(),
            )
