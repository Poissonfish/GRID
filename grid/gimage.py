...
class GImage:
    ...
    def crop(self, pts=None):
        ...
        self.set(key="crop", value=imgCrop)
        self.set(key="mean", value=self.get("crop")[:, :, :3].mean(axis=2))
    ...
    def doKMeans(self, k=3, features=[0, 1, 2], colorOnly=False):
        ...
        elif (k != self.paramKMs["k"]) or (features != self.paramKMs["features"]):
            # Will skip if no updates on the params
            imgK, center = doKMeans(img=self.get("crop"), k=k, features=features)
    ...
    def binarize(self, k=3, features=[0, 1, 2], lsSelect=[0]):
        ...
        if len(features) == 0:
            ...
            (k != self.paramKMs["k"])
            or (features != self.paramKMs["features"])
            or (lsSelect != self.paramKMs["lsSelect"])
        ):
            ...
            self.set(
                key="binOrg",
                value=((np.isin(self.get("kmean"), clusterSelected)) * 1).astype(int),
            )
    ...
    def smooth(self, value):
        ...
        if value != self.paramKMs["valSmth"]:
            ...
            self.set(
                key="binTemp",
                value=smoothImg(image=self.get(key="binTemp"), n=valSmthReal),
            )
    ...
    def finalized(self):
        ...
        self.set(key="bin", value=np.multiply(self.get("binSm"), self.get("binSd")))
    ...
    def readyForSeg(self):
        ...
        self.set(key="binSeg", value=blurImg(self.get("bin"), n=nSmt))
    ...
    def rankCenters(self, k, center, colorOnly=False):
        ...
        if colorOnly:
            ratioK = [
                (center[i, 0] - center[i, 1]) / center[i, :].sum()
            ...
            for i in range(k):
                ...
                try:
                    scPeaks = round(
                        len(find_peaks(sigs, height=(sigs.mean()))[0])
                        / len(find_peaks(sigs)[0]),
                        4,
                    )
                except Exception:
                    scPeaks = 0

                score = scMaxF * 0.25 + scMean * 0.25 + scPeaks * 0.5
                scores.append(score)
